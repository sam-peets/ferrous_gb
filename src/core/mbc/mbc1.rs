...
impl Mbc for Mbc1 {
    fn read(&self, addr: u16) -> u8 {
        ...
        match addr {
            0x0000..=0x3fff => match self.mode {
                0 => self.rom[0][addr],
                1 => self.rom[(bank2 << 5) % self.rom_banks][addr],
                _ => unreachable!("Mbc1: invalid addressing mode"),
            },
            0x4000..=0x7fff => self.rom[((bank2 << 5) | bank1) % self.rom_banks][addr - 0x4000],
            0xa000..=0xbfff => {
                if self.ram_enable {
                    match self.mode {
                        0 => self.ram[0][addr - 0xa000],
                        1 => self.ram[bank2 % self.ram_banks][addr - 0xa000],
                        _ => unreachable!("Mbc1: invalid addressing mode"),
                    }
                } else {
                    0xff
            ...
            _ => {
                log::warn!("Mbc1: invalid read: {addr:04x?}");
                0xff
            }
        }
    }
    ...
    fn write(&mut self, addr: u16, val: u8) {
        ...
        match addr {
            ...
            _ => {
                log::warn!("Mbc1: invalid write: {addr:04x?}");
                // discard the write
            }
        }
    }
